...
//    FILE *debugFile;
    debugFile=fopen("debug.txt", "a");
    char text[128];
    sprintf(text, "%d,%d,%d,",phonemeindex, phonemeLength, stress);
...
int Parser1()
{
    ...
    FILE *debugFile;
    debugFile=fopen("debug.txt", "a");
    fprintf(debugFile, "Clear the stress table.\n");
    fclose(debugFile);
	...
	while((sign1 = input[srcpos]) != 155) { // 155 (\233) is end of line marker
        ...
        if ((match = full_match(sign1, sign2)) != -1) {
            ...
            if (match == 0){
                    FILE *debugFile;
                    debugFile=fopen("debug.txt", "a");
                    fprintf(debugFile, "Parser 1 failed, no match");
                    fclose(debugFile);
